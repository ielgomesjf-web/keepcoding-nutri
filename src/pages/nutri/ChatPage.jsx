...
export default function ChatPage() {
  ...
  return (
    <div className="animate-fade-in h-[calc(100vh-7rem)]">
      <h1 className="font-heading text-xl sm:text-2xl font-bold text-text-primary mb-4 sm:mb-6">Chat</h1>

      <div className="flex h-[calc(100%-3rem)] gap-0 md:gap-4">
        {/* Conversation List */}
        <Card padding="none" className={`flex flex-col w-full md:w-80 md:shrink-0 ${active ? 'hidden md:flex' : 'flex'}`}>
          <div className="p-3 border-b border-border">
            <div className="relative">
              ...
              <input
                ...
                className="w-full pl-9 pr-3 py-2.5 rounded-lg bg-gray-50 border-none text-sm outline-none"
              />
            </div>
          </div>
          <div className="flex-1 overflow-y-auto">
            {conversations.filter(c => c.name.toLowerCase().includes(searchQuery.toLowerCase())).map(conv => (
              <button key={conv.id} onClick={() => setActive(conv.id)}
                className={`w-full flex items-center gap-3 px-4 py-3.5 text-left transition-colors cursor-pointer ${active === conv.id ? 'bg-primary/5 border-l-2 border-primary' : 'hover:bg-gray-50 border-l-2 border-transparent'}`}>
                <div className="relative">
                ...
                {conv.unread > 0 && (
                  <span className="w-5 h-5 rounded-full bg-primary text-white text-[11px] flex items-center justify-center shrink-0">{conv.unread}</span>
                )}
              </button>
            ))}
          </div>
        ...
        {/* Chat Area */}
        <Card padding="none" className={`flex-1 flex flex-col ${!active ? 'hidden md:flex' : 'flex'}`}>
          {active && activeConv ? (
            <>
              <div className="flex items-center gap-3 px-3 sm:px-5 py-3 border-b border-border">
                <button onClick={() => setActive(null)} className="md:hidden p-2 -ml-2 text-text-secondary cursor-pointer">
                  <ArrowLeft size={20} />
                </button>
                <div className="relative">
              ...
              <div className="flex-1 overflow-y-auto px-3 sm:px-5 py-4 flex flex-col gap-3">
                {msgs.map(msg => (
                  <div key={msg.id} className={`flex ${msg.from === 'nutri' ? 'justify-end' : 'justify-start'}`}>
                    <div className={`max-w-[80%] sm:max-w-[70%] px-4 py-2.5 rounded-2xl text-sm ${
              ...
              <div className="px-3 sm:px-4 py-3 border-t border-border flex items-end gap-2">
                <button className="p-3 text-text-muted hover:text-text-secondary cursor-pointer"><Paperclip size={18} /></button>
                <textarea
                  value={newMsg}
                  ...
                  rows={1}
                  className="flex-1 px-4 py-2.5 rounded-lg bg-gray-50 border-none text-sm outline-none resize-none"
                />
